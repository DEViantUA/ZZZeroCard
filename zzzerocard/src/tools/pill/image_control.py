...
async def get_image_from_boost_speed(link):
    path = f"/boost_speed/{link.split('https://')[1]}"
...
async def open_image(image_data, link = ""):
    try:
...
async def get_download_img(link, size=None, thumbnail_size=None):
    ...
    if image_boost is None:
        ...
        if _boost_speed:
            if "hoyoverse" in link:
                full_path = os.path.join(assets, f"/boost_speed/{link.split('https://')[1]}".lstrip('/'))
                await save_image(image, full_path)
            
        image = await open_image(image, link)
    else:
        image = image_boost
